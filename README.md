...
The full user guide lives in **[`docs/`](docs/README.md)** — a 17-chapter, example-driven guide covering everything from getting started to the API reference, all verified against the current codebase and the 366-test automated suite. See also **[`CHANGELOG.md`](CHANGELOG.md)** for release notes.
...
Run the full automated test suite (366 tests: 300 unit, 28 end-to-end, and 38 MCP contract tests):
```bash
...
* **Protected Execution (`POST /mcp`)**: Authenticated JSON-RPC 2.0 interface exposing **59 tools** across workflow design, context bindings, Copilot synthesis, time-travel replay, state machine verification, execution, human tasks, notifications, and advisory agents.
