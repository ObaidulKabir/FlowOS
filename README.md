...
*   **MCP server**: `flowos-mcp` (Streamable HTTP on host port **8081** → `POST /mcp`; set `MCP_API_KEY` and `MCP_ROLE` before Compose; stdio remains available via `MCP_TRANSPORT=stdio`)
