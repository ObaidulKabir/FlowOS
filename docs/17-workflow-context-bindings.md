...
Decision auto-route (`conditions.Default` or `true`) moves `currentStep` without consuming a state-machine event. If the template still declares `Assigned + QUOTE_APPROVED → Quoted`, include `QUOTE_APPROVED` in the simulation event list. FlowOS applies it as state-only catch-up (step unchanged, state advances). Omitting it leaves state at Assigned and the next event is Denied. Bindings never create tenant roles: map `roleOverrides` to existing roles or declare no template roles. Agents should read MCP resource `flowos://guides/dual-kernel-design` or prompt `design_dual_kernel_workflow`.
