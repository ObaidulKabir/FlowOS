      ...
      {activeTab === 'browser' && (
        <div className="bg-slate-900/80 rounded-xl p-4 border border-slate-800 text-xs">
          ...
          <ol className="list-decimal list-inside space-y-2 text-slate-300 leading-relaxed">
            ...
            <li>
              <strong>AI task automation:</strong> Call MCP prompt <code className="text-blue-300">automate_waiting_task_with_ai_agent</code> or read <code className="text-blue-300">flowos://guides/ai-task-automation</code>. Design the waiting step with <code className="text-blue-300">design_agent_handled_step</code>. Keep a waiting HumanTask/Command (never a Decision <code className="text-blue-300">Default</code> skip). Set <code className="text-blue-300">actor: Agent</code> or <code className="text-blue-300">Either</code>. Create the prompt with <code className="text-blue-300">upsert_agent_prompt</code> and the tenant LLM with <code className="text-blue-300">upsert_agent_provider</code> (write-only key), then point the step with <code className="text-blue-300">agentPrompt</code> / <code className="text-blue-300">agentProvider</code>. Dashboard: Application → AI Context → Prompts / Providers — not the top Keys tab. Inspect with <code className="text-blue-300">preview_agent_context</code> or <code className="text-blue-300">get_agent_context</code> (neither runs the model). <code className="text-blue-300">simulate_*</code> never calls the live LLM. On a live instance, publish the human gate then <code className="text-blue-300">run_agent_task</code>; use <code className="text-blue-300">suggest_agent_action</code> only as advisory. Do not free-form <code className="text-blue-300">publish_event</code> to impersonate the agent.
            </li>
            <li>
