      ...
      {activeTab === 'browser' && (
        <div className="bg-slate-900/80 rounded-xl p-4 border border-slate-800 text-xs">
          ...
          <ol className="list-decimal list-inside space-y-2 text-slate-300 leading-relaxed">
            ...
            <li>
              <strong>Bounded-autonomy tasks:</strong> Call MCP prompt <code className="text-blue-300">design_agent_handled_step</code> or read <code className="text-blue-300">flowos://guides/bounded-autonomy-tasks</code>. Keep a waiting HumanTask/Command (never a Decision <code className="text-blue-300">Default</code> skip). Set <code className="text-blue-300">actor: Agent</code> or <code className="text-blue-300">Either</code>. Create/edit the prompt with <code className="text-blue-300">upsert_agent_prompt</code> (or dashboard <code className="text-blue-300">Agent Prompts</code>), then point the step with <code className="text-blue-300">agentPrompt</code>. Inspect composed Prompt/Data/Tools/Provider with <code className="text-blue-300">preview_agent_context</code> (design-time) or <code className="text-blue-300">get_agent_context</code> (live) — those do not run the agent. Put tenant policy on binding <code className="text-blue-300">policyGuideline</code>, and auto-commit only events listed on both <code className="text-blue-300">nextSteps</code> and <code className="text-blue-300">autoCommit.allowedEvents</code>. Register the tenant model with <code className="text-blue-300">register_plugin_binding</code> (<code className="text-blue-300">bindingType: agent</code>, write-only <code className="text-blue-300">apiKey</code>), point the step at it with <code className="text-blue-300">agentProvider</code>, and declare <code className="text-blue-300">agentTools</code> from existing plugins/capabilities. Use <code className="text-blue-300">suggest_agent_action</code> to run the agent without publishing and <code className="text-blue-300">run_agent_task</code> to host the loop — do not free-form <code className="text-blue-300">publish_event</code>.
            </li>
            <li>
              <strong>Tenant resource tools:</strong> Expose tenant APIs with <code className="text-blue-300">register_capability_binding</code>, then declare them on the waiting step as <code className="text-blue-300">LookupRecord:crm.customer.get.v1</code>, <code className="text-blue-300">QueryRecords:</code>, <code className="text-blue-300">FetchDocument:</code>, <code className="text-blue-300">SearchKnowledge:</code>, or <code className="text-blue-300">CheckPolicy:</code>. FlowOS prefetches those reads into Agent Context. Write capabilities (<code className="text-blue-300">capability:payment.refund.v1</code>) and notify plugins are listed but not prefetched. The model never sees the URL.
            </li>
            <li>
